    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert([{
            ...
            sender_email: formData.recipient_email, // Saved for notification logic
      ...
      <div className="w-full max-w-lg bg-white/40 backdrop-blur-xl border border-white/40 rounded-3xl shadow-2xl p-8 relative z-10">
        ...
        {step === 2 && (
          <div className="animate-in fade-in slide-in-from-right-4 duration-500">
            ...
            <input type="email" placeholder="Your Email (for notification)" className="w-full p-4 bg-white/60 border border-rose-100 rounded-2xl outline-none" value={formData.recipient_email} onChange={(e) => setFormData({...formData, recipient_email: e.target.value})} />
            ...
            <div className="grid grid-cols-3 gap-3 mb-6">
              {previews.map((url, index) => (
                <div key={index} className="relative aspect-square">
                  ...
                  <button onClick={() => removeImage(index)} className="absolute -top-1 -right-1 bg-rose-500 text-white rounded-full w-5 h-5 text-xs">✕</button>
                </div>
              ))}
              <label className="aspect-square border-2 border-dashed border-rose-200 rounded-xl flex items-center justify-center cursor-pointer hover:bg-white/40">
        ...
        {step === 5 && (
          <div className="animate-in zoom-in-95 duration-500">
            ...
            <div className="p-6 bg-white/80 rounded-2xl shadow-inner border border-rose-50">
                ...
                <p className="text-sm font-bold text-rose-500 text-right">— {formData.from}</p>
            </div>
            <button onClick={handleSealAndSend} className="w-full mt-8 bg-rose-600 text-white py-4 rounded-2xl font-bold shadow-xl">Seal & Send ❤️</button>
          </div>
        )}
        ...
        {step === 6 && (
          <div className="text-center animate-in zoom-in-95 duration-500">
            ...
            <div className="p-4 bg-rose-50 border-2 border-dashed border-rose-200 rounded-2xl mb-6">
              ...
              <code className="text-[10px] break-all text-rose-800 bg-white p-2 rounded block mb-4">{generatedLink}</code>
              <button onClick={() => { navigator.clipboard.writeText(generatedLink); alert("Copied!"); }} className="bg-rose-500 text-white px-6 py-2 rounded-xl text-sm font-bold">Copy Link</button>
            </div>
